...
const colorStore = (function () {
	 ...
	 */
	const set = (color: ColorObject | undefined) => {
		if (!color) return;
		console.log(color)
		const objectWithMaterial = getObjectWithMaterial(get(selectedUUID));
		if (!objectWithMaterial) return;
 ...
 * 
 * @param {HSVColor | HSVTupleColor} val HSV Parsable. See {@link HSVColor}, {@link HSVTupleColor}, {@link hueNumber}, {@link percentNumber}		 * @returns {Color | undefined} An object with HSL, HSV, HEX, and RGB values, or undefined if failed. See {@link ColorObject}  
 ...
 * @returns {Color | undefined} An object with HSL, HSV, HEX, and RGB values, or undefined if failed. See {@link ColorObject}  
